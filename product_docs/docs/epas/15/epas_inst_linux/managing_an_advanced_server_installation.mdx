...
To initialize a cluster with the non-default values, you can use the `PGSETUP_INITDB_OPTIONS` environment variable by invoking the `edb-as-15-setup` cluster initialization script that resides under `EPAS_Home/bin`.
...
!!! Note
    The data directory is created under `/var/lib/edb-as/15/main2` and configuration directory is created under `/etc/edb-as/15/main/`.
...
By default, data files reside under `/var/lib/edb/as15/data` directory. To use a data directory that resides in a non-default location, perform the following steps:
...
-   Delete the entire content of `/etc/systemd/system/edb-as-15.service` file, except the following line:
...
By default, the data files resides under `/var/lib/edb/as15/data` directory. To change the default data directory location depending on individual environment preferences, you must configure the SELinux policy and perform the following steps:
...
-   Create a file `edb-as-15.service` under `/etc/systemd/system` directory to include the location of a new data directory:
...
The `edb-as15-server-core` RPM for version 7.x | 8.x contains a unit file that starts the EDB Postgres Advanced Server instance. The file allows you to start multiple services, with unique `data` directories and that monitor different ports. You must have `root` access to invoke or modify the script.
