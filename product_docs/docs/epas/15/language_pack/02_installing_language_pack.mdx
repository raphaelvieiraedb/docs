...
```shell
set PATH=C:\edb\languagepack\v2\Python-3.7;
C:\edb\languagepack\v2\Perl-5.26\bin;
C:\edb\languagepack\v2\Tcl-8.6\bin;%PATH%
```

After performing the steps required to configure Language Pack on Windows, use the Windows `Services` applet to restart the EDB Postgres Advanced Server.
