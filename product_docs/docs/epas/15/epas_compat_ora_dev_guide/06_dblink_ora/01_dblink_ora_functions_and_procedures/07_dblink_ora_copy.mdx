...
-   `count` instructs the server to report status information every `n` record, where `n` is the number specified. During the execution of the function, EDB Postgres Advanced Server raises a notice of severity `INFO` with each iteration of the count. For example, if FeedbackCount is `10, dblink_ora_copy()` raises a notice every `10` records. `count` is optional; if omitted, the value is `0`.
